...
Personal AI Scheduling Assistant is an AI workflow automation app, not just a calendar chatbot. A user writes a natural-language plan, the backend extracts structured tasks, validates constraints, runs an algorithmic scheduler, detects conflicts, and returns a calendar-ready timeline with suggestions. The important design choice is that the LLM only handles understanding; deterministic backend logic owns scheduling correctness.
...
3. AI task understanding: uses an OpenAI-compatible extraction service when configured, and a fallback parser for local demos.
